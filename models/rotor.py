...
class Rotor(Component):
    def __init__(self, config):
        ...
        self.a2 = config["a2"]
        self.b2 = 1.0-self.a2
        self.h_hub = (self.wl_hub - self.wl_hg)/12.0
    ...
    def get_force_and_moment(self, state, controls, environment):
        ...
        velocity_dot = state.get("velocity_dot", math.array([0.0, 0.0, 0.0]))
        ...
        gam_om_16 = rho*self.a_mr*self.c_mr*self.r_mr**4/i_b*omega_mr/16.0*(1.0+8.0/3.0*self.e_mr/self.r_mr)
        ...
        for i in range(4):
            ...
            vi_mr = math.sqrt(math.abs(vi_mr_2))

        p_induced_mr = thrust_mr*vi_mr
        p_climb = self.wt*(-velocity_dot)
